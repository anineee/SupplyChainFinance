...
contract SupplyChainContract2 {
	...
	event showReceiptEvent(string from, string to, int256 value, uint return_time, int status, bool used);
...
//		receipts[receipts_num] = Receipt(from, to, value, return_time, 0, false);
		...
		emit showReceiptEvent(from, to, value, return_time, 0, false);
	}
	...
	*		>=0		回执的数组下标
	*/
	function getReceipt(string memory from, string memory to) public returns(uint256) {
		for(uint256 i = 0; i < receipts_num; i ++) {
			if(utilCompareInternal(receipts[i].from, from)
				&& utilCompareInternal(receipts[i].to, to)){
				emit showReceiptEvent(receipts[i].from, receipts[i].to, receipts[i].value, receipts[i].return_time, receipts[i].status, receipts[i].used);
				return i;
			}	
		}
		return (uint256)(-1);
	...
	*		r_idx	回执的数组下标
	*/
	function checkReceiptSign(uint256 r_idx) public {
	...
	*		-2		调用者不是付款公司	
	*/
	function registerReceipt(string memory from, string memory to, int256 value, uint return_time) public returns(int) {
		uint256 from_idx = getCompanyIdx(from);
		if(from_idx == (uint256)(-1))
			return -1;
	...
	*		-3		调用者不是付款公司	
	*/
	function transferReceipt(string memory from, string memory to, int256 value) public returns(int){
		uint256 from_idx = getCompanyIdx(from);
		if(from_idx == (uint256)(-1))
			return -1;
